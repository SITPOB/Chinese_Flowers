...
public class Register extends F_demo{
    ...
    private void RegisterUI() {
        ...
        register.setContentAreaFilled(false);//设置为透明
        ...
        if (name.equals("") || user.equals("") || phone_num.equals("") || password.equals("") || password2.equals("")) {
            ...
            try{
                ...
                if (identity.equals("用户")) {
                    ...
                    //判断用户名是否存在
                    PreparedStatement pstmt_1 = conn.prepareStatement("select UserID from userinfo where UserAccount = '" + user + "'");
                    ...
                    if (rs.next()) {
                        ...
                        PreparedStatement pstme_2 = conn.prepareStatement("insert into userinfo(UserAccount,UserPWD,NickName,IsActive,UserPhone,UserAge) values(?,?,?,?,?,?)");//创建游标2
                        ...
                        ResultSet rs_1 = stmt.executeQuery("SELECT UserID FROM userinfo where UserAccount = '" + user + "'");
                        int id = rs_1.getInt("UserID");
                        stmt.close();
                        rs_1.close();
                        JOptionPane.showMessageDialog(null, "注册成功！", "提示", JOptionPane.INFORMATION_MESSAGE);//
                        user_model.setAll(id, age_str, user, password, name, phone_num);
                        dlcg = true;
                        if(mainFrame1 != null){
                            mainFrame1.dispose();
                        }
